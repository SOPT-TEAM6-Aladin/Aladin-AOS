...
class HomeTopsellerAdapter : ListAdapter<Topic, HomeTopsellerAdapter.TopSellerViewHolder>(
    topSellerDiffCallback
) {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TopSellerViewHolder {
        val binding =
            ItemHomeTopsellerBinding.inflate(
    ...
    class TopSellerViewHolder(
        val binding: ItemHomeTopsellerBinding
    ) :
        RecyclerView.ViewHolder(binding.root) {
        fun onBind(book: Topic) {
            Glide.with(this.binding.root)
    ...
    companion object {
        private val topSellerDiffCallback = ItemDiffCallback<Topic>(
